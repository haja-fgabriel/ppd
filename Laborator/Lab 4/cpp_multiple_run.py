...
def log_to_excel(filename, time, num_threads, num_polynomes, max_degree, num_monomes):
    workbook = None
    if os.path.exists(filename):
        workbook = openpyxl.load_workbook(filename)
    else:
	    workbook = openpyxl.Workbook()
	    workbook.active.append(('Run no.', 'Language', 'Execution time', 'No. of threads', 'No. polynomes', 'Max degree', 'No. monomes'))
    sheet1 = workbook.active
    
    last_run_no = sheet1.max_row
    sheet1.append((last_run_no, 'C++', time, num_threads, num_polynomes, max_degree, num_monomes))
   
    workbook.save(filename)

if __name__ == '__main__':
    if len(sys.argv) < len(ARGS_STRINGS) + 1:
        exit(f"Usage: {sys.argv[0]} " + ' '.join(ARGS_STRINGS) + " [<num_threads>] ")
	...
	]  ) +  ([sys.argv[6]] if len(sys.argv) > (NUM_ARGS + 1) else [])  + [
		"--log-to-socket"
	]
    ...
    log_to_excel(sys.argv[1], avg_time, '1' if len(sys.argv) == NUM_ARGS+1 else sys.argv[6], sys.argv[2], sys.argv[3], sys.argv[4])