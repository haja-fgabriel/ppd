...
)
{
    ...
    for (int i = 0; i < NThreads; i++)
    {
        ...
        threads.push_back(thread(TransformMatrix<T>, Input, KeInput, N, M, KeN, KeM, start, end, barrier));
        start = end;
    }
    for (int i = 0; i < NThreads; i++)
    {
...
int main(int argc, char* argv[])
{
    ...
    if (strcmp(argv[1], "dynamic") == 0)
    {
        ...
        tBegin = clock();
        HaveFun(nThreads, input, kernelInput, n, m, keN, keM);
        tEnd = clock();
    ...
    else if (strcmp(argv[1], "static") == 0)
    {
        ...
        tBegin = clock();
        HaveFun(nThreads, gInput, gKernelInput, n, m, keN, keM);
        tEnd = clock();
